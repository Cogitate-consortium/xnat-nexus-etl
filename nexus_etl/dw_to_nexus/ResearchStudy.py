...
from dw_dataclasses.research_study import research_study
...
postgres_table_name = 'research_study'
...
df = convert_datatypes_based_on_table('research_study', df) 
df = df.replace({np.nan:None})

current_ts = datetime.datetime.now()

# validate that the data structure matches the dataclass defn
validate_structure(research_study, df)
...
for index, row in df.iterrows():
    ...
    try:
        nexus_dict = nexus.resources.fetch(org, project, row.research_study_uri)
    except:
        nexus_dict = None
