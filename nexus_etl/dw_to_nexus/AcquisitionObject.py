...
from dw_dataclasses.acquisition_object import acquisition_object
...
postgres_table_name = 'acquisition_object'
resource_type = 'nidm:AcquisitionObject'
...
query = f"""
            ...
            and (
                dicom_header is not null
	            or non_dicom_header is not null
            )
        """
...
validate_structure(acquisition_object, df)
...
for index, row in df.iterrows():
    ...
    try:
        nexus_dict = nexus.resources.fetch(org, project, row.acquisition_object_uri)
    except:
        nexus_dict = None
